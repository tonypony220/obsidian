...
## Контракт = порт на границе процессов

Wire-контракт (JSON shapes + коды ошибок, OpenAPI, protobuf) — тот же driving-порт, только видимый **снаружи процесса**: описание того, как внешний мир зовёт приложение. Суть общая: обе стороны зависят от договора, не друг от друга. Разница — в арбитре:

| | Порт (внутри процесса) | Wire-контракт (между процессами) |
|---|---|---|
| Форма | interface / сигнатура функции | JSON shape, коды ошибок, схема |
| Кто проверяет | компилятор, бесплатно | **никто по умолчанию** — компилятор через провод не достаёт |
| Чем чинят | не собирается | schema + codegen, zod-парс на границе, contract-тесты |

Ручная копия контрактных типов на consumer-стороне = договор без арбитра, drift не ловится → producer-пины и boundary parse из [[type-driven-design]].

Обобщение шире: порт, wire-контракт, ABI, syscall-интерфейс, сетевой протокол — везде «интерфейс как договор границы», реализации по обе стороны сменные.
