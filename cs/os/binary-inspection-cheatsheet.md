...
|---|---|
| `nm -n f` | символы по возрастанию адресов. `U` = дырка, `T/t` = код, `d` = .data, `b` = .bss, `a` = константа-equ. РЕГИСТР = видимость: заглавная — глобальный символ, строчная — приватный (`static`, [[c-module-pattern]]) |
